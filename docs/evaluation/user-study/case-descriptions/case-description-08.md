...
* Used template: dynamic for each project
...
* I: With the help of the supervisor, the participant can find the additional fields in `Considered Options`.
* O: Participant fills in the fields in  `Good, because...` and `Bad, because...`.
* O: Participant commits and pushs the ADR.
...
* Include or link graphics in Markdown.
...
* Sufficiently well
* For new users, more explanations in the UI would be helpful, e.g., through explanations on hover, and modes should be explained.
...
1. Functional Completeness (4), Functional Correctness (4), Functional Appropriateness (4)
2. Learnability (4), Operability (4), Aesthetics (3)
3. Templates are dynamic and the used tools depend on the team but, in general, the tool could be used in this field.
