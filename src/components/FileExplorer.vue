<template>
  <v-card flat class="text-left d-flex flex-column" style="height: 100%;">
                ...
                <v-tooltip bottom open-delay="500">
                  <template v-slot:activator="{ on, attrs }">
                    ...
                    <v-list-item-content v-bind="attrs" v-on="on">
                      <v-list-item-title v-text="dispayInfo(file)">
                      </v-list-item-title>
                    </v-list-item-content>
                  </template>
    ...
    <div class="flex-grow-0 d-flex flex-wrap">
      ...
      <DialogCommit
        v-bind:currUser="userName"
        v-bind:currRepo="repoName"
        v-bind:listFilesChanged="listFilesChanged"
      >
        <template v-slot:activator="{ on, attrs }">
          <v-btn v-bind="attrs" v-on="on" class="flex-grow-1 secondary">
            Commit and Push
  ...
  methods: {
    ...
    addRepositories(repoList) {
      if (typeof repoList !== "undefined") {
        ...
        console.log(
          "Something went wrong while adding repos to the file explorer! These aren't valid repos:",
          repoList
        );
      }
    },
    ...
    removeRepository(repo) {
      ...
      Object.entries(this.listFilesChanged).forEach(([key, value]) => {
        let repoName =
          value.path.split("/")[0] + "/" + value.path.split("/")[1];
        if (repoName === repo.fullName) {
          delete this.listFilesChanged[key];
        }
      });
    },
