          ...
          <v-list-item-group data-cy="repoNameList">
            <v-list-group v-for="(repo, i) in folderTree" :key="repo.path"
              ...
              <template v-slot:activator>
                ...
                <DialogCommit v-bind:repo="{ name: repo.name }" v-if="repo.fileType === 'repo'">
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn style="width: 30px; min-width: 30px; height: 100%" class="mx-0 px-0" v-bind="attrs"
                      v-on="on">
                ...
                <DialogRemoveRepository v-bind:repo="{ name: repo.name }" v-if="repo.fileType === 'repo'"
                  ...
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn data-cy="removeRepo" style="width: 30px; min-width: 30px; height: 100%" class="mx-0 px-0"
              ...
              <v-list-item-group v-model="openAdrPath">
                <v-list-item data-cy="adrList" v-for="file in repo.children" :key="file.path"
                  ...
                  <v-list-item-icon>
                    <DialogDeleteAdr :adr="file.adr" :repo="repo.repository">
                      <template v-slot:activator="{ on, attrs }">
                        <v-btn data-cy="deleteAdrBtn" style="width: 30px; min-width: 30px; height: 100%"
                          class="mx-0 px-0" v-if="
                ...
                <div class="d-flex justify-content-space-around justify-center">
                  <v-btn data-cy="newADR" dark class="align-center" width="75%" @click.native.prevent.stop="
    ...
    methods: {
      ...
      openFile({ file }) {
        if (typeof file.adr !== "undefined" && store.currentlyEditedAdr !== file.adr) {
          store.openAdrBy(file.path.split("/")[0] + "/" + file.path.split("/")[1], file.adr.path.split("/").pop());
        } else {
