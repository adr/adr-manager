  ...
  methods: {
    ...
    computeFolderStructureFromData({data, basePath, repository}) {
      ...
      data.forEach(function addToStructure(dataEntry) {
        const path = dataEntry.path.split('/');
        const last = path.pop();
        let parent = fS;
        for (var i in path) {
    ...
    folderContainsPath({folder, path}) {
      const searchedDirectory = getFileByPath({folder: folder, path: folder.path + '/' + path});
    ...
    openFileByPath({path}) {
      ...
      const file = getFileByPath({folder: this.folderStructure, path});
